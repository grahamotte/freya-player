...
- Changing the protocol does not update the port value. Changing from HTTPS to HTTP should set the port to the Jellyfin HTTP port (8096), and changing from HTTP to HTTPS should set the port to 443. Today only the placeholder changes; the port value stays as entered/defaulted.
