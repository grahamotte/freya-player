...
4. Run `mise deploy:publish` and follow it until every configured target finishes. If the user requested preparation without review, run `mise deploy:publish --no-review`; do not submit any App Store version for review. Let the task perform its own push and publishing stages; do not reproduce stages manually or edit its cache. If App Store Connect is still processing a build, wait and rerun the same command so it can resume.
